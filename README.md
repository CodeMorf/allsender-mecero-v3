...
- Caja: el historial consulta `/pos/cash-register/sessions` y el reporte de cierre/X/Z usa `POST /pos/cash-register/sessions/{id}/print`. El backend valida la sucursal/tenant, carga los movimientos completos y resuelve primero la impresora activa del terminal/order place de esa sucursal, con fallback a la impresora `directPrint` predeterminada de la sucursal; después genera el comprobante, lo encola en `print_jobs` y conserva el fallback de impresión del navegador si la red o la cola no están disponibles. El Z incluye totales, arqueo, denominaciones cuando existen y el historial de movimientos.
