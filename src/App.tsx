...
import { Bell, CalendarDays, Check, ChefHat, ChevronLeft, ChevronRight, ClipboardList, CloudOff, Clock, CreditCard, Delete, Divide, Lock, LogOut, Map, Menu, Minus, Moon, Plus, Printer, Search, Sun, Truck, Unlock, UserCheck, UserCircle2, UserX, UtensilsCrossed, Wallet, Wifi, X } from 'lucide-react'
...
function PinScreen({ brand, branch, role, onRoleChange, offline, loading, error, notice, onSubmit, canChangeBranch, onBack }: { brand: string; branch: string; role: StaffRole; onRoleChange: (role: StaffRole) => void; offline: boolean; loading: boolean; error: string; notice: string; onSubmit: (pin: string) => void; canChangeBranch: boolean; onBack: () => void }) {
  ...
  return <main className="pin-shell pin-shell-modern">
    ...
    <section className="pin-card pin-card-modern">
      <div className="pin-brand-block"><div className="brand-mark"><UtensilsCrossed size={28} /></div><div className="pin-business-type"><UtensilsCrossed size={15} /> RESTAURANTE</div><h1>{brand || 'RestaPP'}</h1><p>{branch || 'Sucursal autorizada'}</p></div>
