...
function KitchenPanel({ offline, places, standalone = false, allowAll = true, onClose, onUpdateStatus }: { offline: boolean; places: KitchenPlace[]; standalone?: boolean; allowAll?: boolean; onClose: () => void; onUpdateStatus: (kotId: number, status: string, idempotencyKey: string) => Promise<{ queued: boolean; message: string }> }) {
  const [tickets, setTickets] = useState<KitchenTicket[]>([])
  const [loading, setLoading] = useState(!offline)
  const [error, setError] = useState('')
  const [busyId, setBusyId] = useState<number | null>(null)
  const [selectedPlaceId, setSelectedPlaceId] = useState<number | 'all'>(() => allowAll ? 'all' : places.find(place => place.isDefault)?.id || places[0]?.id || 'all')
