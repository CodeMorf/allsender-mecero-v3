...
async function playWaiterAlert(settings: NotificationSettings = defaultNotificationSettings) {
  ...
  try {
    ...
    await LocalNotifications.schedule({ notifications: [{ id: Date.now() % 2147483647, title: 'Llamada de mesa', body: 'Hay una llamada pendiente en sala.', channelId: 'waiter-calls', sound: settings.sound === 'none' ? undefined : 'service_bell.mp3' }] })
  } catch {
...
export default function App() {
  ...
  async function hydrate(session: Session) {
    ...
    try {
      const [remoteTables, remoteItems, config, remoteOrders, remoteKots, remoteNotifications, remoteReceiptSettings, remotePrinters, remotePaymentMethods] = await Promise.all([
        api.tables('pin'),
        api.menuItems('pin'),
...
function KitchenPanel({ offline, onClose, onUpdateStatus }: { offline: boolean; onClose: () => void; onUpdateStatus: (kotId: number, status: string, idempotencyKey: string) => Promise<{ queued: boolean; message: string }> }) {
  const [tickets, setTickets] = useState<KitchenTicket[]>([])
  const [loading, setLoading] = useState(!offline)
  const [error, setError] = useState('')
  const [busyId, setBusyId] = useState<number | null>(null)
  async function load() {
    if (offline) { setTickets((readCache().kots || []).filter(ticket => ['pending_confirmation', 'in_kitchen', 'food_ready'].includes(ticket.status))); setLoading(false); return }
    setLoading(true); setError('')
    try {
      const values = await api.kots('pin')
      saveCache({ kots: values }); setTickets(values.filter(ticket => ['pending_confirmation', 'in_kitchen', 'food_ready'].includes(ticket.status)))
  ...
  // eslint-disable-next-line react-hooks/set-state-in-effect, react-hooks/exhaustive-deps
  useEffect(() => { void load(); if (offline) return; const timer = window.setInterval(() => void load(), 8_000); return () => window.clearInterval(timer) }, [offline])
  ...
  return <div className="modal-backdrop"><section className="modal wide kitchen-panel"><header><div><p className="eyebrow">AUTORIZACIÓN DE COCINA</p><h2>Cocina · pedidos activos</h2><small>Las nuevas comandas aparecen aquí sin repetir las anteriores.</small></div><div className="kitchen-header-actions"><button className="button outline" onClick={() => void load()} disabled={loading || offline}>Actualizar</button><button className="icon-button" onClick={onClose}><X /></button></div></header>{offline && <Alert>Sin conexión: los cambios quedan guardados localmente y se sincronizarán al restablecerse la conexión.</Alert>}{error && <Alert>{error}</Alert>}{loading ? <div className="empty compact"><p>Cargando pedidos activos…</p></div> : tickets.length ? <div className="kitchen-list">{tickets.map(ticket => <article className={`kitchen-ticket kitchen-${ticket.status}`} key={ticket.id}><div className="kitchen-ticket-header"><div><strong>{kitchenTicketLabel(ticket.kotNumber, ticket.id)}</strong><small>{orderNumberLabel(ticket.orderNumber, `Pedido n.º ${ticket.orderId}`)}{ticket.tableName ? ` · Mesa ${ticket.tableName}` : ''}{ticket.kitchenPlace ? ` · ${ticket.kitchenPlace}` : ''}</small></div><span className="kitchen-status">{statusLabel(ticket.status)}</span></div><ul>{ticket.items.map(item => <li key={item.id}><strong>{item.quantity}× {item.name}</strong>{item.variation && <small> · {item.variation}</small>}{item.modifiers?.length ? <small> · {item.modifiers.map(modifier => modifier.name).join(', ')}</small> : null}{item.note && <em>{item.note}</em>}</li>)}</ul>{ticket.note && <p className="kitchen-note">{ticket.note}</p>}<footer><small>{ticket.createdAt ? new Date(ticket.createdAt).toLocaleTimeString('es-DO', { hour: '2-digit', minute: '2-digit' }) : 'Hora no publicada'}</small><button className="button primary" disabled={busyId !== null} onClick={() => void advance(ticket)}>{busyId === ticket.id ? 'Guardando…' : actionLabel(ticket.status)}</button></footer></article>)}</div> : <div className="empty compact"><ChefHat size={34} /><p>No hay pedidos pendientes, en preparación o listos.</p></div>}<footer className="modal-note">El personal de cocina actualiza cada pedido; el personal de sala ve el estado en el mapa y los avisos. Agregar artículos a una orden existente crea una comanda adicional solo para las líneas nuevas.</footer></section></div>
}
